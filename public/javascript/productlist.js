      ...
      .then((res) => {
        ...
        res.data.forEach((product) => {
          if (!product.deletionStatus) {
            let card = `<a class="col-xl-2 col-lg-3 col-md-4 col-sm-4 col-6  d-flex justify-content-center text-decoration-none text-dark" onclick="window.location.href='/products/product-detail/${
              product._id
            }/mainimage'">
...
async function addToWhishListinProducts(event, productId, userId, btn, doing) {
  ...
  if (doing == "add") {
    ...
    if (res.status) {
      ...
      btn.setAttribute(
        "onclick",
        `addToWhishListinProducts(event, ${productId}, ${userId}, this, 'remove')`
      );
    ...
    fetch(
      ...
      .then((res) => {
        if (res.status) {
          btn.setAttribute(
            "onclick",
            `addToWhishListinProducts(event,${productId},${userId},this,'add')`
          );
          btn.style.boxShadow = "";
